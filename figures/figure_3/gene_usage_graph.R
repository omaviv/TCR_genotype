...
v_frequencies$GENE <- gsub("TRBV6-23", "TRBV6-2/TRBV6-3", v_frequencies$GENE)
...
v_usage_graph <- ggplot(v_frequencies, aes(x = GENE, y = FREQ)) +  
  ...
  # scale_color_manual(values = safe_colorblind_palette[1:9]) +
  # geom_jitter(aes(colour = factor(D2_GENO)), size = 1.5) +
  scale_x_discrete(limits=TRBV_LOC) + 
  scale_y_continuous(expand = c(0, 0), limits = c(0,max(v_frequencies$FREQ)*1.15)) +
  labs(color="Gene family") +
  ylab("Usage") +
  guides(colour=guide_legend(nrow = 2)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        text = element_text(size=20), axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
...
dj_usage_graph <- ggplot(ALL_DJ_GENO, aes(x = GENE, y = FREQ)) +  
  geom_boxplot(alpha = 0.80, fill=NA) +
  geom_jitter(size = 1.5) +
  # geom_jitter(aes(colour = factor(FAMILY)), size = 1.5)  + scale_color_brewer(palette = "Set2") +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,max(ALL_DJ_GENO$FREQ)*1.15)) +
  # labs(color="Gene\ngroup") +
  # labs(color="TRBD2\ngenotype") +
  ylab("Usage") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
...
ggsave(paste0(figure_folder, "DS1_TRBDJ_usage_colored_by_group.pdf"), dj_usage_graph, height = 8, width = 5.35)
...
p1.common.y <- ggplot_gtable(ggplot_build(v_usage_graph))
p2.common.y <- ggplot_gtable(ggplot_build(dj_usage_graph))

# copy the plot height from p1 to p2
p2.common.y$heights <- p1.common.y$heights

usage_plot <- grid.arrange(p1.common.y, p2.common.y, ncol=2, widths=c(0.77,0.23))

ggsave(paste0(figure_folder, "DS1_TRB_usage.pdf"), usage_plot, height = 8, width = 20)
