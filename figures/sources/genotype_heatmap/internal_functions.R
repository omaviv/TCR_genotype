...
#' @export
sortDFByGene <- function(DATA, chain = c("IGH", "IGK", "IGL", "TRB", "TRA"), method = c("name", "position"), removeIGH = FALSE, geno = FALSE, peseudo_remove = F, ORF_remove = F) {
  if (missing(chain)) {
    chain = "IGH"
  }
  ...
  if (method == "name") {
    ...
    if (removeIGH) {
      DATA$GENE <- gsub("IG[H|K|L]|TR[B|A]", "", DATA$GENE)
      DATA$GENE <- factor(DATA$GENE, levels = rev(sortAlleles(unique(DATA$GENE), method = method)))
      if(!geno) DATA$hapBy <- gsub("IG[H|K|L]|TR[B|A]", "", DATA$hapBy)
    }
  } else {
    DATA$GENE <- factor(DATA$GENE, levels = rev(GENE.loc.tmp))
    if (removeIGH) {
      GENE.loc.tmp <- gsub("IG[H|K|L]|TR[B|A]", "", GENE.loc.tmp)
      names(GENE.loc.tmp) <- GENE.loc.tmp
      DATA$GENE <- gsub("IG[H|K|L]|TR[B|A]", "", DATA$GENE, perl = T)
      DATA$GENE <- factor(DATA$GENE, levels = rev(GENE.loc.tmp))
      if(!geno) DATA$hapBy <- gsub("IG[H|K|L]|TR[B|A]", "", DATA$hapBy)
    }
  }
...
#'
#' @export
sortGenotype <- function(geno_table, chain = c("IGH", "IGK", "IGL", "TRB", "TRA"), gene_sort = "position", removeIGH = TRUE, lk_cutoff = 1){
  if (missing(chain)) {
    chain = "IGH"
  }
  chain <- match.arg(chain)
