...
#'
#' @export
genoHeatmap <- function(geno_table, chain = c("IGH", "IGK", "IGL", "TRB", "TRA"), gene_sort = "position", removeIGH = TRUE, lk_cutoff = 1, mark_low_lk = TRUE, html = FALSE, n_line = 4, line_length=60, pseudo_genes = FALSE, ORF_genes = FALSE, file = NULL, color_y = NULL) {
  if (missing(chain)) {
    chain = "IGH"
  }
  ...
  # select columns
  geno_db <- geno_table[,c("SUBJECT", "GENE", "GENOTYPED_ALLELES", "K_DIFF")]
  # rename the columns
  ...
  val_novel <- grep('^[0-9]+[_].',geno_db_m$ALLELES, value = T)
  novel <- F
  novel_allele_text <- c()
  novel_symbol <- "\u005E"
  ...
  Map(axis, side=2, at=(0:(samples_n-1))/(axis_loc_div), col.axis=colors, labels=rownames(m), lwd=0, las=1, cex.axis=2) #left
  ...
  grid(lwd=2,ny = nrow(m2), nx = 0,col = "black",lty = 2)
