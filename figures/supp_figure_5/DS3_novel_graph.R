...
figure_folder <- paste0(project_folder, "figures/supp_figure_5/")
...
#             paste0(figure_folder, "BIOMED2_TRBV_with_novel.fasta"), open="w")
# 
# write.fasta(sequences=as.list(gsub(TRBV_GERM, pattern = '.', replacement = '', fixed = T)),
#             names=names(TRBV_GERM),
#             paste0(figure_folder, "BIOMED2_TRBV_with_novel_ref.fasta"), open="w")
...
high_freq_novel_df <- NOVEL_DIS_DF[NOVEL_DIS_DF$SUBJECTS_NUM >= 20,]
high_freq_allele_order <- high_freq_novel_df$FULL_ALLELE_NAME[order(-high_freq_novel_df$SUBJECTS_NUM)]

low_freq_novel_df <- NOVEL_DIS_DF[NOVEL_DIS_DF$SUBJECTS_NUM < 20,]
...
colors <- unlist(lapply(low_freq_allele_order, function(allele) {ifelse(novel_checks$padded[novel_checks$ALLELE==allele], "red", "black")}))
